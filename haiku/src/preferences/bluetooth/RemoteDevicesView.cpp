 ...
 :	BView(name, flags)
{
	...
	servicesButton = new BButton("services",
...
RemoteDevicesView::MessageReceived(BMessage* message)
{
	switch (message->what) {
		...
		case kMsgQueryServices:
		{
			...
			BluetoothDevice* btDev = device->Device();
			if (btDev == NULL)
				break;

			bdaddr_t addr = btDev->GetBluetoothAddress();
			BString name = btDev->GetFriendlyName();
