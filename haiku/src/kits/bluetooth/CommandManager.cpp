...
void*
buildSetEventMask(uint8 mask[8], size_t* outsize)
{
	CALLED();
...
void*
buildWriteClassOfDevice(uint8 devClass[3], size_t* outsize)
{
	CALLED();
	struct hci_write_dev_class* param;
	void* command = buildCommand(OGF_CONTROL_BASEBAND, OCF_WRITE_CLASS_OF_DEV,
		(void**)&param, sizeof(struct hci_write_dev_class), outsize);

	if (command != NULL)
		memcpy(param->dev_class, devClass, 3);
...
void*
buildWriteStoredLinkKey(bdaddr_t bdaddr, const uint8 key[16], size_t* outsize)
{
	CALLED();
	struct hci_write_stored_link_key* param;
	void* command = buildCommand(OGF_CONTROL_BASEBAND,
		OCF_WRITE_STORED_LINK_KEY, (void**)&param,
		sizeof(struct hci_write_stored_link_key), outsize);

	if (command != NULL) {
		param->num_keys_to_write = 1;
		param->bdaddr = bdaddr;
		memcpy(param->key, key, HCI_LINK_KEY_SIZE);
	}
...
void*
buildDeleteStoredLinkKey(bool deleteAll, size_t* outsize)
{
	CALLED();
	struct hci_delete_stored_link_key* param;
	void* command = buildCommand(OGF_CONTROL_BASEBAND,
		OCF_DELETE_STORED_LINK_KEY, (void**)&param,
		sizeof(struct hci_delete_stored_link_key), outsize);

	if (command != NULL) {
		memset(&param->bdaddr, 0, sizeof(param->bdaddr));
		param->delete_all_flag = deleteAll ? 0x01 : 0x00;
	}
...
buildAuthenticationRequested(uint16 handle, size_t* outsize)
{
	...
	void* command = buildCommand(OGF_LINK_CONTROL, OCF_AUTH_REQUESTED,
		(void**)&param, sizeof(struct hci_cp_auth_requested), outsize);
...
void*
buildSetConnectionEncryption(uint16 handle, uint8 enable, size_t* outsize)
{
	CALLED();
	struct hci_cp_set_conn_encrypt* param;
	void* command = buildCommand(OGF_LINK_CONTROL, OCF_SET_CONN_ENCRYPT,
		(void**)&param, sizeof(struct hci_cp_set_conn_encrypt), outsize);

	if (command != NULL) {
		param->handle = B_HOST_TO_LENDIAN_INT16(handle);
		param->encrypt = enable;
	}
...
#if 0
#pragma mark - LE CONTROL -
#endif


void*
buildLeSetEventMask(uint8 mask[8], size_t* outsize)
{
	CALLED();
...
void*
buildLeReadBufferSize(size_t* outsize)
{
	CALLED();
	return buildCommand(OGF_LE_CONTROL, OCF_LE_READ_BUFFER_SIZE,
		NULL, 0, outsize);
}
...
{
	CALLED();
	struct hci_cp_le_set_scan_params* param;
	void* command = buildCommand(OGF_LE_CONTROL, OCF_LE_SET_SCAN_PARAMS,
		(void**)&param, sizeof(struct hci_cp_le_set_scan_params), outsize);
...
void*
buildLeSetScanEnable(uint8 enable, uint8 filterDup, size_t* outsize)
{
	CALLED();
...
void*
buildLeCreateConnection(uint16 scanInterval, uint16 scanWindow,
...
{
	CALLED();
	struct hci_cp_le_create_conn* param;
	void* command = buildCommand(OGF_LE_CONTROL, OCF_LE_CREATE_CONN,
		(void**)&param, sizeof(struct hci_cp_le_create_conn), outsize);

	if (command != NULL) {
		param->scan_interval = B_HOST_TO_LENDIAN_INT16(scanInterval);
		param->scan_window = B_HOST_TO_LENDIAN_INT16(scanWindow);
		param->filter_policy = filterPolicy;
		param->peer_address_type = peerAddressType;
		param->peer_address = peerAddress;
		param->own_address_type = ownAddressType;
		param->conn_interval_min = B_HOST_TO_LENDIAN_INT16(connIntervalMin);
		param->conn_interval_max = B_HOST_TO_LENDIAN_INT16(connIntervalMax);
		param->conn_latency = B_HOST_TO_LENDIAN_INT16(connLatency);
		param->supervision_timeout = B_HOST_TO_LENDIAN_INT16(supervisionTimeout);
		param->min_ce_length = 0;
		param->max_ce_length = 0;
	}
...
void*
buildLeCreateConnectionCancel(size_t* outsize)
{
	CALLED();
	return buildCommand(OGF_LE_CONTROL, OCF_LE_CREATE_CONN_CANCEL,
		NULL, 0, outsize);
}


void*
buildLeStartEncryption(uint16 handle, const uint8 random[8],
	uint16 ediv, const uint8 ltk[16], size_t* outsize)
{
	CALLED();
	struct hci_cp_le_start_encryption* param;
	void* command = buildCommand(OGF_LE_CONTROL, OCF_LE_START_ENCRYPTION,
		(void**)&param, sizeof(struct hci_cp_le_start_encryption), outsize);

	if (command != NULL) {
		param->handle = B_HOST_TO_LENDIAN_INT16(handle);
		memcpy(param->random, random, 8);
		param->ediv = B_HOST_TO_LENDIAN_INT16(ediv);
		memcpy(param->ltk, ltk, 16);
	}
...
void*
buildLeSetDataLength(uint16 handle, uint16 txOctets,
	uint16 txTime, size_t* outsize)
{
	CALLED();
	struct hci_cp_le_set_data_length* param;
	void* command = buildCommand(OGF_LE_CONTROL, OCF_LE_SET_DATA_LENGTH,
		(void**)&param, sizeof(struct hci_cp_le_set_data_length), outsize);

	if (command != NULL) {
		param->handle = B_HOST_TO_LENDIAN_INT16(handle);
		param->tx_octets = B_HOST_TO_LENDIAN_INT16(txOctets);
		param->tx_time = B_HOST_TO_LENDIAN_INT16(txTime);
	}
...
void*
buildLeLtkRequestReply(uint16 handle, const uint8 ltk[16],
	size_t* outsize)
{
	CALLED();
	struct hci_cp_le_ltk_request_reply* param;
	void* command = buildCommand(OGF_LE_CONTROL, OCF_LE_LTK_REQUEST_REPLY,
		(void**)&param, sizeof(struct hci_cp_le_ltk_request_reply), outsize);
...
void*
buildLeLtkRequestNegReply(uint16 handle, size_t* outsize)
{
	CALLED();
	struct hci_cp_le_ltk_request_neg_reply* param;
	void* command = buildCommand(OGF_LE_CONTROL, OCF_LE_LTK_REQUEST_NEG_REPLY,
		(void**)&param, sizeof(struct hci_cp_le_ltk_request_neg_reply),
		outsize);
...
#if 0
#pragma mark - EIR (Extended Inquiry Response) -
#endif


void*
buildWriteExtendedInquiryResponse(const uint16* uuids16, uint8 uuidCount,
	const char* name, size_t* outsize)
{
	CALLED();
	struct hci_write_ext_inquiry_rsp* param;
	void* command = buildCommand(OGF_CONTROL_BASEBAND,
		OCF_WRITE_EXT_INQUIRY_RSP,
		(void**)&param, sizeof(struct hci_write_ext_inquiry_rsp), outsize);

	if (command != NULL) {
		param->fec_required = 0x00;	// FEC not required
		memset(param->data, 0, HCI_EIR_MAX_DATA_SIZE);

		uint8* p = param->data;
		size_t remaining = HCI_EIR_MAX_DATA_SIZE;

		// Complete List of 16-bit Service Class UUIDs
		if (uuidCount > 0 && remaining >= (size_t)(2 + uuidCount * 2)) {
			p[0] = 1 + uuidCount * 2;	// length
			p[1] = HCI_EIR_UUID16_COMPLETE;
			for (uint8 i = 0; i < uuidCount; i++) {
				// UUIDs in EIR are little-endian
				p[2 + i * 2] = (uint8)(uuids16[i] & 0xFF);
				p[2 + i * 2 + 1] = (uint8)(uuids16[i] >> 8);
			}
			size_t entryLen = 2 + uuidCount * 2;
			p += entryLen;
			remaining -= entryLen;
		}
		...
		if (name != NULL && name[0] != '\0') {
			...
			if (nameLen > remaining - 2)
				nameLen = remaining - 2;
			if (remaining >= 2 + nameLen) {
				p[0] = 1 + nameLen;	// length
				p[1] = HCI_EIR_NAME_COMPLETE;
				memcpy(p + 2, name, nameLen);
				p += 2 + nameLen;
				remaining -= 2 + nameLen;
			}
		}
...
#if 0
#pragma mark - SSP (Secure Simple Pairing) -
#endif


void*
buildWriteSimplePairingMode(uint8 mode, size_t* outsize)
{
	CALLED();
	struct hci_write_simple_pairing_mode* param;
	void* command = buildCommand(OGF_CONTROL_BASEBAND,
		OCF_WRITE_SIMPLE_PAIRING_MODE,
		(void**)&param, sizeof(struct hci_write_simple_pairing_mode), outsize);

	if (command != NULL)
		param->simple_pairing_mode = mode;
...
void*
buildIoCapabilityRequestReply(bdaddr_t bdaddr, uint8 ioCapability,
	uint8 oobDataPresent, uint8 authRequirements, size_t* outsize)
{
	...
	void* command = buildCommand(OGF_LINK_CONTROL,
		...
		(void**)&param, sizeof(struct hci_cp_io_capability_request_reply),
		outsize);

	if (command != NULL) {
		param->bdaddr = bdaddr;
		param->io_capability = ioCapability;
		param->oob_data_present = oobDataPresent;
		param->auth_requirements = authRequirements;
	}
...
void*
buildUserConfirmationRequestReply(bdaddr_t bdaddr, size_t* outsize)
{
	CALLED();
	struct hci_cp_user_confirmation_request_reply* param;
	void* command = buildCommand(OGF_LINK_CONTROL,
		OCF_USER_CONFIRMATION_REQUEST_REPLY,
		(void**)&param,
		sizeof(struct hci_cp_user_confirmation_request_reply), outsize);
...
void*
buildUserConfirmationRequestNegReply(bdaddr_t bdaddr, size_t* outsize)
{
	CALLED();
	struct hci_cp_user_confirmation_neg_reply* param;
	void* command = buildCommand(OGF_LINK_CONTROL,
		OCF_USER_CONFIRMATION_NEG_REPLY,
		(void**)&param,
		sizeof(struct hci_cp_user_confirmation_neg_reply), outsize);
...
const char* leControlCommands[] = {
	...
	"LE Read Remote Features",					/* 0x0016 */
	"LE Encrypt",								/* 0x0017 */
	"LE Rand",									/* 0x0018 */
	"LE Start Encryption",						/* 0x0019 */
	"LE Long Term Key Request Reply",			/* 0x001A */
	"LE Long Term Key Request Negative Reply",	/* 0x001B */
...
BluetoothCommandOpcode(uint16 opcode)
{
	...
	switch (GET_OPCODE_OGF(opcode)) {
		...
		case OGF_LE_CONTROL:
		{
			uint16 ocf = GET_OPCODE_OCF(opcode);
			if (ocf >= 1
				&& ocf <= sizeof(leControlCommands) / sizeof(const char*))
				return leControlCommands[ocf - 1];
			return "Unknown LE command";
		}
...
BluetoothEvent(uint8 event)
{
	...
	if (event >= HCI_HAIKU_EVENT_SERVER_QUITTING)
		return "Haiku Internal Event";
	if (event >= 1
		&& event <= sizeof(bluetoothEvents) / sizeof(const char*))
