...
status_t
LocalDevice::GetPairedDevices(BMessage* outList)
{
	BMessenger* messenger = _RetrieveBluetoothMessenger();
	if (messenger == NULL)
		return B_ERROR;
	...
	status_t status = messenger->SendMessage(&request, outList);
	delete messenger;
...
status_t
LocalDevice::SaveDeviceName(const bdaddr_t& addr, const char* name,
	uint32 deviceClass)
{
	BMessenger* messenger = _RetrieveBluetoothMessenger();
	if (messenger == NULL)
	...
	BMessage request(BT_MSG_SAVE_DEVICE_NAME);
	BString addrStr;
	addrStr.SetToFormat("%02X:%02X:%02X:%02X:%02X:%02X",
		addr.b[5], addr.b[4], addr.b[3],
		addr.b[2], addr.b[1], addr.b[0]);
	request.AddString("bdaddr", addrStr);
	request.AddString("name", name);
	if (deviceClass != 0)
		request.AddInt32("cod", (int32)deviceClass);
	...
	fHid(hid)
{
	...
	// Set Event Mask to include SSP events (bits 48-53)
	// Default mask 0x00001FFFFFFFFFFF + SSP bits → 0x003F1FFFFFFFFFFF
	{
		uint8 eventMask[8] = {
			0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x1F, 0x3F, 0x00
		};
		size_t size;
		void* command = buildSetEventMask(eventMask, &size);
		if (command != NULL) {
			BMessage request(BT_MSG_HANDLE_SIMPLE_REQUEST);
			BMessage reply;
			request.AddInt32("hci_id", fHid);
	...
	// Enable Secure Simple Pairing (required for BT 2.1+)
	{
		size_t size;
		void* command = buildWriteSimplePairingMode(0x01, &size);
		if (command != NULL) {
			BMessage request(BT_MSG_HANDLE_SIMPLE_REQUEST);
			BMessage reply;
			request.AddInt32("hci_id", fHid);
			...
			if (fMessenger->SendMessage(&request, &reply) == B_OK) {
				...
				if (reply.FindInt8("status", &status) == B_OK
					&& status != BT_OK) {
					printf("LocalDevice: Write Simple Pairing Mode "
						"failed status=%d\n", status);
				}
			}
			free(command);
	...
	// Write Extended Inquiry Response with SPP UUID so Android discovers
	// our Serial Port service during inquiry.
	{
		uint16 uuids[] = { 0x1101, 0x112E };	// SPP + PBAP PCE
		size_t size;
		void* command = buildWriteExtendedInquiryResponse(uuids, 2,
			"Haiku", &size);
		if (command != NULL) {
			BMessage request(BT_MSG_HANDLE_SIMPLE_REQUEST);
			BMessage reply;
			...
			if (fMessenger->SendMessage(&request, &reply) == B_OK) {
				...
				if (reply.FindInt8("status", &status) == B_OK
					&& status != BT_OK) {
					printf("LocalDevice: Write Extended Inquiry Response "
						"failed status=%d\n", status);
				}
			}
			free(command);
