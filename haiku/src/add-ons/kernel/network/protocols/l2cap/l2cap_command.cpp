	...
	l2cap_rfc* rfc, int8 fcsOption)
{
	...
	// Options are prepended (added to the front of the buffer), so we must
	// add them in REVERSE order to get the correct wire order.
	// Desired wire order: MTU, flush_timeout, flow, RFC, FCS
	// Prepend order: FCS, RFC, flow, flush_timeout, MTU

	if (fcsOption >= 0) {
		struct config_option_fcs {
			l2cap_configuration_option header;
			uint8 value;
		} _PACKED;
		NetBufferPrepend<config_option_fcs> option(buffer.Get());
		if (option.Status() != B_OK)
			return NULL;

		option->header.type = l2cap_configuration_option::OPTION_FCS;
		option->header.length = sizeof(option->value);
		option->value = (uint8)fcsOption;
	}

	if (rfc != NULL) {
		struct config_option_rfc {
			l2cap_configuration_option header;
			l2cap_rfc value;
		} _PACKED;
		NetBufferPrepend<config_option_rfc> option(buffer.Get());
		if (option.Status() != B_OK)
			return NULL;

		option->header.type = l2cap_configuration_option::OPTION_RFC;
		option->header.length = sizeof(option->value);
		option->value.mode = rfc->mode;
		option->value.tx_window_size = rfc->tx_window_size;
		option->value.max_transmit = rfc->max_transmit;
		option->value.retransmission_timeout = htole16(rfc->retransmission_timeout);
		option->value.monitor_timeout = htole16(rfc->monitor_timeout);
		option->value.max_pdu_size = htole16(rfc->max_pdu_size);
	}
...
make_l2cap_information_rsp(uint8& code, uint16 type, uint16 result, uint16 _mtu)
{
	...
	if (result == l2cap_information_rsp::RESULT_SUCCESS) {
		...
		case l2cap_information_req::TYPE_EXTENDED_FEATURES: {
			uint32 features = htole32(_mtu);
			gBufferModule->append(buffer.Get(), &features, sizeof(features));
			break;
		}
		case l2cap_information_req::TYPE_FIXED_CHANNELS: {
			uint64 channels = B_HOST_TO_LENDIAN_INT64((uint64)_mtu);
			gBufferModule->append(buffer.Get(), &channels, sizeof(channels));
			break;
		}
		}
	}
