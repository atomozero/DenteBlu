...
l2cap_receive_data(net_buffer* buffer)
{
	...
	ERROR("l2cap: recv len=%d cid=0x%04x\n", length, dcid);
	...
	switch (dcid) {
		case L2CAP_SIGNALING_CID:
		{
			...
			if (connection == NULL) {
				ERROR("l2cap: no connection for received L2CAP signaling command\n");
				gBufferModule->free(buffer);
				return ENOTCONN;
			}
		...
		case L2CAP_ATT_CID:
		{
			struct HciConnection* connection = connection_for(buffer);
			if (connection == NULL) {
				ERROR("l2cap: no connection for ATT packet\n");
				gBufferModule->free(buffer);
				return ENOTCONN;
			}
			status = att_receive_data(connection, buffer);
			break;
		}
		...
		case L2CAP_LE_SIGNALING_CID:
		{
			struct HciConnection* connection = connection_for(buffer);
			if (connection == NULL) {
				ERROR("l2cap: no connection for LE signaling packet\n");
				gBufferModule->free(buffer);
				return ENOTCONN;
			}
			status = l2cap_handle_le_signaling_command(connection, buffer);
			break;
		}
		...
		case L2CAP_SMP_CID:
		{
			struct HciConnection* connection = connection_for(buffer);
			if (connection == NULL) {
				ERROR("l2cap: no connection for SMP packet\n");
				gBufferModule->free(buffer);
				return ENOTCONN;
			}
			status = smp_receive_data(connection, buffer);
			break;
		}
...
l2cap_error_received(net_error error, net_error_data* errorData, net_buffer* data)
{
	...
	if (error == B_NET_ERROR_UNREACH_HOST) {
		// The connection pointer is embedded in the buffer data by
		// ~HciConnection, because connection_for() can't find the
		// connection (it's already removed from the list at this point).
		HciConnection* connection = NULL;
		if (data->size >= sizeof(void*))
			gBufferModule->read(data, 0, &connection, sizeof(connection));

		if (connection != NULL)
			gL2capEndpointManager.Disconnected(connection);

		data->interface_address = NULL;
