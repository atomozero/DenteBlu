...
L2capEndpointManager::Disconnected(HciConnection* connection)
{
	...
	while (iter.HasNext()) {
		...
		// Wake any thread blocked in ReadData() → fifo_dequeue_buffer().
		net_buffer* eofMarker = gBufferModule->create(0);
		if (eofMarker != NULL)
			gStackModule->fifo_enqueue_buffer(&endpoint->fReceiveQueue, eofMarker);

		endpoint->fCommandWait.NotifyAll(B_ERROR);
	}
}
