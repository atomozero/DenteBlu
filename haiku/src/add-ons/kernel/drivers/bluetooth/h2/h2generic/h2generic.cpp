...
device_added(usb_device dev, void** cookie)
{
	...
	*/
	...
	// Intel Bluetooth firmware loading
	if (desc->vendor_id == USB_VENDOR_INTEL) {
		// Retry limit: after firmware download the chip reboots its USB
		// interface, causing device_added to be called again.  If the
		// firmware loaded successfully, btintel_setup returns B_OK
		// (img_type == operational).  If it keeps returning
		// B_SHUTTING_DOWN we stop after a few attempts.
		static int32 sIntelFwAttempts = 0;
		static const int32 kMaxIntelFwAttempts = 3;

		if (sIntelFwAttempts >= kMaxIntelFwAttempts) {
			ERROR("%s: Intel firmware setup failed after %" B_PRId32
				" attempts, giving up — not registering bootloader"
				" device\n", __func__, sIntelFwAttempts);
			err = B_DEV_NOT_READY;
			goto bail;
		} else {
			TRACE("%s: Intel BT device detected, firmware setup "
				"attempt %" B_PRId32 "\n", __func__,
				sIntelFwAttempts + 1);
			...
			if (intelStatus == B_SHUTTING_DOWN) {
				...
				*cookie = new_bt_dev;
				...
				sIntelFwAttempts++;
				ERROR("%s: Intel firmware setup failed: %s "
					"(attempt %" B_PRId32 "/%" B_PRId32 ") "
					"— not registering bootloader device\n",
					__func__, strerror(intelStatus),
					sIntelFwAttempts, kMaxIntelFwAttempts);
				err = intelStatus;
				goto bail;
			}
		}
...
device_control(void* cookie, uint32 msg, void* params, size_t size)
{
	...
	switch (msg) {
		...
		case BT_UP:
			dprintf("bt BT_UP: bdev=%p eventRoom=%p aclRoom=%p "
				"snetRecycle=%p nbuffersTx[0]=%p nbuffersTx[1]=%p\n",
				bdev, &bdev->eventRoom, &bdev->aclRoom,
				&bdev->snetBufferRecycleTrash,
				&bdev->nbuffersTx[0], &bdev->nbuffersTx[1]);
			//  EVENTS
	...
	{
		if (size == 0 || btDevices == NULL
			|| btDevices->HandleLeGattIoctl == NULL) {
			err = B_BAD_VALUE;
			break;
		}

		void* kparams = malloc(size);
		if (kparams == NULL) {
			err = B_NO_MEMORY;
			break;
		}

		if (user_memcpy(kparams, params, size) != B_OK) {
			free(kparams);
			err = B_BAD_ADDRESS;
			break;
		}

		hci_id hdev = bdev->hdev;

		// Release device lock — GATT operations block waiting for
		// ATT responses and must not hold the device lock
		release_sem(bdev->lock);

		err = btDevices->HandleLeGattIoctl(hdev, msg, kparams, size);

		acquire_sem(bdev->lock);

		if (err == B_OK) {
			if (user_memcpy(params, kparams, size) != B_OK)
				err = B_BAD_ADDRESS;
		}
...
init_driver(void)
{
	...
	add_debugger_command("bttrace", &dump_bt_trace,
		"Dumps BT USB buffer trace ring");
...
uninit_driver(void)
{
	...
	remove_debugger_command("bttrace", &dump_bt_trace);
