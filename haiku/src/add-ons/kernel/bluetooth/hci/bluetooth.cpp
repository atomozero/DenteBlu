...
bt_hci_module_info* btDevices = NULL; // self-reference for AttChannel

extern bt_hci_module_info sBluetoothModule;
...
RegisterDriver(bt_hci_transport_hooks* hooks, bluetooth_device** _device)
{
	...
	// HCI ACL Data Packet Length — the maximum L2CAP fragment size
	// that fits in a single HCI ACL packet. Most BR/EDR controllers
	// report 1021 via Read_Buffer_Size. Use 1021 as default until
	// we plumb the actual value from BluetoothServer.
	device->mtu = 1021;
...
// Lazy-init AttChannel and GattClient for a LE connection
static status_t
EnsureAttChannel(HciConnection* conn)
{
	if (conn->attChannel != NULL)
		return B_OK;

	AttChannel* attCh = new(std::nothrow) AttChannel(conn);
	if (attCh == NULL)
		return B_NO_MEMORY;

	conn->attChannel = attCh;

	TRACE("%s: AttChannel created for handle=%#x\n", __func__,
		conn->handle);
	return B_OK;
}
...
static status_t
EnsureSmpManager(HciConnection* conn)
{
	if (conn->smpManager != NULL)
		return B_OK;

	conn->smpManager = new(std::nothrow) SmpManager(conn);
	return conn->smpManager != NULL ? B_OK : B_NO_MEMORY;
}


status_t
HandleLeGattIoctl(hci_id hciId, uint32 op, void* data, size_t size)
{
	if (data == NULL)
		return B_BAD_VALUE;

	// All GATT param structs start with uint16 conn_handle
	uint16 connHandle = *(uint16*)data;

	HciConnection* conn = btCoreData->ConnectionByHandle(connHandle, hciId);
	if (conn == NULL) {
		ERROR("%s: no connection for handle=%#x hid=%" B_PRId32 "\n",
			__func__, connHandle, hciId);
		return B_DEV_BAD_DRIVE_NUM;
	}

	// Lazy-init AttChannel
	status_t status = EnsureAttChannel(conn);
	if (status != B_OK)
		return status;

	AttChannel* attCh = (AttChannel*)conn->attChannel;
	GattClient gatt(attCh);

	switch (op) {
		case BT_LE_GATT_EXCHANGE_MTU:
		{
			struct bt_le_gatt_mtu_params* params
				= (struct bt_le_gatt_mtu_params*)data;
			uint16 serverMtu;
			params->status = attCh->ExchangeMtu(params->desired_mtu,
				&serverMtu);
			params->negotiated_mtu = attCh->Mtu();
			return B_OK;
		}

		case BT_LE_GATT_DISCOVER_SERVICES:
		{
			struct bt_le_gatt_discover_services_params* params
				= (struct bt_le_gatt_discover_services_params*)data;
			params->count = BT_GATT_MAX_SERVICES;
			params->status = gatt.DiscoverPrimaryServices(
				params->services, &params->count);
			return B_OK;
		}

		case BT_LE_GATT_DISCOVER_CHARACTERISTICS:
		{
			struct bt_le_gatt_discover_chars_params* params
				= (struct bt_le_gatt_discover_chars_params*)data;
			params->count = BT_GATT_MAX_CHARS;
			params->status = gatt.DiscoverCharacteristics(
				params->start_handle, params->end_handle,
				params->chars, &params->count);
			return B_OK;
		}

		case BT_LE_GATT_DISCOVER_DESCRIPTORS:
		{
			struct bt_le_gatt_discover_descriptors_params* params
				= (struct bt_le_gatt_discover_descriptors_params*)data;
			params->status = gatt.DiscoverDescriptors(
				params->start_handle, params->end_handle,
				&params->characteristic);
			return B_OK;
		}

		case BT_LE_GATT_READ:
		{
			struct bt_le_gatt_read_params* params
				= (struct bt_le_gatt_read_params*)data;
			params->value_length = BT_GATT_MAX_VALUE_SIZE;
			params->status = gatt.ReadCharacteristic(
				params->attr_handle, params->value,
				&params->value_length);
			return B_OK;
		}

		case BT_LE_GATT_WRITE:
		{
			struct bt_le_gatt_write_params* params
				= (struct bt_le_gatt_write_params*)data;
			params->status = gatt.WriteCharacteristic(
				params->attr_handle, params->value,
				params->value_length);
			return B_OK;
		}

		case BT_LE_GATT_WRITE_NO_RESPONSE:
		{
			struct bt_le_gatt_write_params* params
				= (struct bt_le_gatt_write_params*)data;
			params->status = gatt.WriteCharacteristicNoResponse(
				params->attr_handle, params->value,
				params->value_length);
			return B_OK;
		}

		case BT_LE_GATT_SUBSCRIBE:
		{
			struct bt_le_gatt_subscribe_params* params
				= (struct bt_le_gatt_subscribe_params*)data;
			params->status = gatt.SubscribeNotifications(
				params->ccc_handle, params->enable);
			return B_OK;
		}

		case BT_LE_SMP_INITIATE_PAIRING:
		{
			struct bt_le_smp_pair_params* params
				= (struct bt_le_smp_pair_params*)data;
			status_t smpStatus = EnsureSmpManager(conn);
			if (smpStatus != B_OK)
				return smpStatus;

			SmpManager* smp = (SmpManager*)conn->smpManager;
			smpStatus = smp->InitiatePairing();
			if (smpStatus == B_OK && params->passkey != (uint32)-1)
				smp->SetPasskey(params->passkey);
			return smpStatus;
		}

		case BT_LE_SMP_NC_CONFIRM:
		{
			struct bt_le_nc_confirm_params* params
				= (struct bt_le_nc_confirm_params*)data;
			if (conn->smpManager == NULL)
				return B_NOT_ALLOWED;
			((SmpManager*)conn->smpManager)->SetNumericComparisonResult(
				params->confirmed);
			return B_OK;
		}
...
bluetooth_std_ops(int32 op, ...)
{
	switch (op) {
		case B_MODULE_INIT:
		{
			...
			// Self-reference so AttChannel can call PostACL
			btDevices = &sBluetoothModule;
