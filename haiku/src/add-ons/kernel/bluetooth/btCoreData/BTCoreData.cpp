...
PostEvent(bluetooth_device* ndev, void* event, size_t size)
{
	...
	switch (outgoingEvent->ecode) {
		...
		case HCI_EVENT_LE_META:
		{
			...
			if (leMeta->subevent == HCI_LE_SUBEVENT_CONN_COMPLETE) {
				struct hci_ev_le_conn_complete* leConn
					= (struct hci_ev_le_conn_complete*)(leMeta + 1);

				if (leConn->status == 0) {
					HciConnection* conn = AddConnection(leConn->handle,
						BT_ACL, leConn->peer_address, ndev->index);

					if (conn == NULL)
						panic("no mem for LE conn desc");
					conn->ndevice = ndev;
					conn->isLE = true;
					conn->leMtu = HCI_LE_DEFAULT_MTU;
					TRACE("%s: Registered LE connection handle=%#x\n",
						__func__, leConn->handle);
				}
			}
			break;
		}
...
bluetooth_core_data_module_info sBCDModule = {
	...
	free_command_ident,
	free_command_idents_by_pointer,
};
