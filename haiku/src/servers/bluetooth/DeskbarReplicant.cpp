...
static const char* kBluetoothDeskbarSignature =
	"application/x-vnd.Haiku-BluetoothDeskbar";

const uint32 kMsgOpenBluetoothPreferences = 'obtp';
const uint32 kMsgSendFile = 'sndf';
...
DeskbarReplicant::Archive(BMessage* archive, bool deep) const
{
	...
	if (status == B_OK)
		status = archive->AddString("add_on", kBluetoothDeskbarSignature);
	if (status == B_OK)
...
DeskbarReplicant::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		...
		case kMsgSendFile:
		{
			// Try by signature first, then fall back to known paths
			status_t err = be_roster->Launch(
				"application/x-vnd.Haiku-BluetoothSendFile");
			if (err != B_OK && err != B_ALREADY_RUNNING) {
				entry_ref ref;
				if (get_ref_for_path(
						"/boot/system/non-packaged/bin/BluetoothSendFile",
						&ref) == B_OK)
...
DeskbarReplicant::MouseDown(BPoint where)
{
	...
	menu->AddItem(new BMenuItem(B_TRANSLATE("Send file" B_UTF8_ELLIPSIS),
...
void
DeskbarReplicant::_ShowErrorAlert(const BString& msg, status_t status)
{
	BString error = B_TRANSLATE("Error: %status%");
	error.ReplaceFirst("%status%", strerror(status));
	BString fullMsg(msg);
	fullMsg << "\n\n" << error;
	BAlert* alert = new BAlert(B_TRANSLATE("Bluetooth error"),
		fullMsg.String(), B_TRANSLATE("OK"));
