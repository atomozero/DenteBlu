...
#define OGF_CONTROL_BASEBAND			0x03
  ...
  /*struct hci_reset {
	...
	struct hci_delete_stored_link_key {
		bdaddr_t	bdaddr;
		uint8		delete_all_flag;  // 0x00=specific, 0x01=all
	} __attribute__ ((packed));
	struct hci_delete_stored_link_key_reply {
		uint8		status;
	...
	// Extended Inquiry Response (OGF_CONTROL_BASEBAND)
	#define OCF_WRITE_EXT_INQUIRY_RSP		0x0052
	#define HCI_EIR_MAX_DATA_SIZE			240
	struct hci_write_ext_inquiry_rsp {
		uint8		fec_required;
		uint8		data[HCI_EIR_MAX_DATA_SIZE];
	} __attribute__ ((packed));

	// EIR Data Type codes
	#define HCI_EIR_FLAGS					0x01
	#define HCI_EIR_UUID16_INCOMPLETE		0x02
	#define HCI_EIR_UUID16_COMPLETE			0x03
	#define HCI_EIR_NAME_SHORT				0x08
	#define HCI_EIR_NAME_COMPLETE			0x09

	// SSP Commands (OGF_CONTROL_BASEBAND)
	...
	struct hci_write_simple_pairing_mode {
		uint8		simple_pairing_mode;
	} __attribute__ ((packed));

	struct hci_read_simple_pairing_mode_reply {
		uint8		status;
		uint8		simple_pairing_mode;
	} __attribute__ ((packed));
	...
	// Synchronous Connection Commands (OGF_LINK_CONTROL)
	...
	#define OCF_REJECT_SYNC_CONN			0x002A
	struct hci_cp_reject_sync_conn {
		bdaddr_t	bdaddr;
		uint8		reason;
	} __attribute__ ((packed));

	// SSP Commands (OGF_LINK_CONTROL)
	...
	struct hci_cp_user_confirmation_request_reply {
		bdaddr_t	bdaddr;
	} __attribute__ ((packed));

	#define OCF_USER_CONFIRMATION_NEG_REPLY		0x002D
	struct hci_cp_user_confirmation_neg_reply {
		bdaddr_t	bdaddr;
	} __attribute__ ((packed));

	#define OCF_USER_PASSKEY_REQUEST_REPLY		0x002E
	struct hci_cp_user_passkey_request_reply {
		bdaddr_t	bdaddr;
		uint32		numeric_value;
	} __attribute__ ((packed));

	#define OCF_USER_PASSKEY_REQUEST_NEG_REPLY	0x002F
